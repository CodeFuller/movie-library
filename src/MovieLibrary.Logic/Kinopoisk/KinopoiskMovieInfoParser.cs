...
namespace MovieLibrary.Logic.Kinopoisk
{
	...
	{
		// For titles in Russian
		private static readonly Regex TitleRegex1 = new Regex(@"<meta property=""og:title"" content=""«(.+?)» ", RegexOptions.Compiled);

		// For titles with no translation to Russian, e.g. https://www.kinopoisk.ru/film/1359229/
		private static readonly Regex TitleRegex2 = new Regex(@"<meta property=""og:title"" content=""(.+?), \d+", RegexOptions.Compiled);
		...
		public MovieInfoModel ParseMovieInfo(string content, Uri sourceUri)
		{
			...
			{
				title ??= ParseValue(line, TitleRegex1) ?? ParseValue(line, TitleRegex2);
